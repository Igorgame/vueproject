...
export default {
    data: () => ({
        links: [        // ссылки на страницы
            {title: 'Главная', url: '/executor', exact: true},
            {title: 'Заказы', url: '/executor/orders'},
            {title: 'О заказе', url: '/executor/order-info'}
        ],
        date: new Date()    //дата
   ...
   computed: {
        date () {  //отображение даты
         ...
         name () { //отображение имени пользователя
            return this.$store.getters.info.name 
        }       
    },
    mounted () {    //интервал для даты
